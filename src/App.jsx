...
const CheckoutModal = ({ isOpen, onClose, cartTotal, onConfirm }) => {
  ...
  const [focusField, setFocusField] = useState('cash'); // 'cash' 或 'discount'
  ...
  const handleQuickDiscount = (percent) => {
    setDiscount(Math.round(cartTotal * (1 - percent)).toString());
  };
      ...
      <div className="bg-white w-full max-w-6xl rounded-[2.5rem] shadow-2xl flex flex-col lg:flex-row overflow-hidden max-h-[95vh] text-slate-900">
        ...
        <div className="lg:w-[35%] bg-slate-50 p-8 border-r border-slate-200 overflow-y-auto">
          ...
          <div className="space-y-6 text-slate-900">
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm text-slate-900">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">原始小計</p>
            ...
            >
              <div className="flex justify-between items-center mb-3">
                ...
                <div className="flex gap-1 text-slate-900">
                  <button type="button" onClick={(e) => { e.stopPropagation(); handleQuickDiscount(0.9); }} className="px-2 py-1 bg-slate-100 rounded-md text-[10px] font-bold hover:bg-blue-100 hover:text-blue-600 transition-colors">9折</button>
                  <button type="button" onClick={(e) => { e.stopPropagation(); handleQuickDiscount(0.8); }} className="px-2 py-1 bg-slate-100 rounded-md text-[10px] font-bold hover:bg-blue-100 hover:text-blue-600 transition-colors">8折</button>
                </div>
              </div>
        ...
        <div className="flex-1 p-8 flex flex-col min-h-0 bg-white">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest">選擇支付方式</h3>
          ...
          <div className="grid grid-cols-3 gap-3 mb-8">
            {[
              { id: 'Cash', label: '現金', icon: <Banknote size={24} /> },
              { id: 'Credit', label: '刷卡', icon: <CreditCard size={24} /> },
              { id: 'Mobile', label: '行動支付', icon: <Smartphone size={24} /> }
            ].map(method => (
              <button
                key={method.id}
                type="button"
          ...
          <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-0">
            <div className="space-y-4">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest text-slate-400">金額確認</h3>
              {paymentMethod === 'Cash' ? (
                <div className="space-y-4">
                  <div
                  ...
                  >
                    <div className="flex justify-between items-center mb-1">
                      <label className="text-[10px] font-bold text-slate-400 uppercase">實收現金</label>
                      <div className="flex gap-1 text-slate-900">
          ...
          <div className="mt-8 flex gap-4 shrink-0">
            <button type="button" onClick={onClose} className="px-8 py-5 rounded-2xl font-bold text-slate-400 hover:bg-slate-50 transition-colors text-slate-400">取消</button>
            <button
              type="button"
...
const Sidebar = () => {
  ...
  return (
    <div className="w-64 h-screen bg-slate-900 text-white fixed left-0 top-0 flex flex-col">
      ...
      >
        <LogOut size={20} />
        <span className="font-bold text-slate-500">安全登出</span>
      </button>
    </div>
  );
};
    ...
    <div className="flex flex-col lg:flex-row gap-8 text-slate-900 h-full overflow-hidden">
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 shrink-0 text-slate-900">
          <div className="flex items-center space-x-4">
            ...
            <div className="bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100 flex items-center font-mono text-blue-600">
              ...
              <span className="font-black text-lg">#{(todayOrderCount + 1).toString().padStart(3, '0')}</span>
            </div>
          </div>
          <div className="relative w-full md:w-64">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 shadow-sm font-medium text-slate-900 text-slate-900"
            />
          </div>
        </div>
        ...
        <div className="flex space-x-2 mb-6 overflow-x-auto pb-2 shrink-0 scrollbar-hide text-slate-900">
          {categories.map(cat => (
            <button
              ...
              className={`px-6 py-2 rounded-full whitespace-nowrap font-bold text-sm transition-all border ${selectedCategory === cat ? 'bg-blue-600 text-white border-blue-600 shadow-md text-white' : 'bg-white text-slate-500 border-slate-200 hover:border-blue-400 text-slate-500'
                }`}
            >
              {cat}
            </button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 overflow-y-auto pr-2 flex-1 pb-10 scrollbar-thin content-start text-slate-900">
          {filteredMenu.map(item => (
            ...
            >
              <div className={`font-bold mb-1 truncate ${item.isAvailable ? 'text-slate-800 group-hover:text-blue-600 text-slate-800' : 'text-slate-400'}`}>
                {item.name}
              </div>
              ...
              {!item.isAvailable && (
                <div className="absolute inset-0 bg-slate-900/5 flex items-center justify-center">
                  <span className="bg-slate-500 text-white text-[10px] px-2 py-0.5 rounded font-bold uppercase tracking-tighter text-white">暫不供應</span>
                </div>
              )}
            </button>
          ))}
      ...
      <div className="w-full lg:w-96 flex-shrink-0 bg-white rounded-3xl shadow-xl flex flex-col border border-slate-100 h-full overflow-hidden text-slate-900">
        <div className="p-6 border-b flex flex-col space-y-4 bg-slate-50/50 rounded-t-3xl shrink-0">
          ...
          <div className="grid grid-cols-2 gap-2 bg-white p-1 rounded-xl border border-slate-200 shadow-inner text-slate-900">
            <button onClick={() => setOrderType('dineIn')} className={`flex items-center justify-center space-x-2 py-2 rounded-lg text-sm font-bold transition-all ${orderType === 'dineIn' ? 'bg-slate-900 text-white shadow-md text-white' : 'text-slate-400 hover:bg-slate-50'}`}><Utensils size={16} /><span>內用</span></button>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-thin text-slate-900">
          {cart.map(item => (
            <div key={item.id} className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-slate-900">
              <div className="flex justify-between items-start mb-2 text-slate-900">
                <span className="font-bold text-slate-700 text-sm text-slate-700">{item.name}</span>
                <span className="font-black text-slate-900 text-sm text-slate-900">${item.price * item.quantity}</span>
              </div>
              <div className="flex justify-between items-center mt-3 text-slate-900">
                <div className="flex items-center space-x-1 bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm text-slate-900 text-slate-900">
          ...
          {cart.length === 0 && <div className="h-full flex flex-col items-center justify-center text-slate-300 opacity-60 text-slate-300"><ShoppingCart size={48} className="mb-2" /><p className="font-medium text-sm text-slate-500">尚未點餐</p></div>}
        </div>

        <div className="p-6 bg-slate-900 text-white rounded-b-3xl shrink-0 text-white text-white">
          <div className="flex justify-between items-center mb-6 text-white">
            <span className="text-slate-400 font-medium text-slate-400">總計金額</span>
...
const OrderManagementPage = () => {
  ...
  const startSettle = (e, order) => {
    e.stopPropagation();
    setActiveOrder(order);
  };

  const confirmSettle = (paymentData) => {
    setOrders(orders.map(o => o.id === activeOrder.id ? { ...o, ...paymentData, paymentStatus: 'paid' } : o));
    setActiveOrder(null);
  };
  ...
  return (
    <div className="max-w-6xl text-slate-900 h-full flex flex-col overflow-hidden text-slate-900">
      <div className="flex justify-between items-end mb-8 shrink-0 text-slate-900">
        ...
        <div className="flex space-x-4 text-slate-900 font-bold">
          <div className="bg-amber-50 px-6 py-3 rounded-2xl border border-amber-100 text-right text-amber-700"><p className="text-[10px] font-bold text-amber-500 uppercase uppercase">待結總額</p><p className="text-xl font-black">${pendingOrders.reduce((s, o) => s + o.total, 0)}</p></div>
          <div className="bg-blue-50 px-6 py-3 rounded-2xl border border-blue-100 text-right text-blue-700"><p className="text-[10px] font-bold text-blue-500 uppercase uppercase">今日已收</p><p className="text-xl font-black">${finishedOrders.reduce((s, o) => s + o.total, 0)}</p></div>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-2 gap-8 overflow-hidden text-slate-900">
        <div className="flex flex-col min-h-0 text-slate-900 text-slate-900">
          <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center shrink-0 text-slate-400">
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-2 scrollbar-thin pb-10 text-slate-900">
            {pendingOrders.map(o => (
              ...
              >
                <div className="flex justify-between items-center text-slate-900">
                  <div>
                    <div className="flex items-center space-x-2 mb-1 text-slate-900 text-slate-900">
                      <span className="text-lg font-black text-slate-800 text-slate-800">#{o.orderNo}</span>
                      <span className="bg-amber-100 text-amber-700 text-[10px] px-2 py-0.5 rounded-full font-bold uppercase">內用後結</span>
                    </div>
                    <div className="text-xs text-slate-400 flex items-center text-slate-400"><Clock size={12} className="mr-1" /> {o.time}</div>
                  </div>
                  <div className="flex items-center space-x-6 text-slate-900">
                    ...
                    <button
                      onClick={(e) => startSettle(e, o)}
                      className="bg-slate-900 text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-600 transition-all flex items-center shadow-md text-white text-white"
                    >
            ...
            {pendingOrders.length === 0 && <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl py-20 text-center text-slate-300 italic font-medium text-slate-300">目前暫無待結訂單</div>}
          </div>
        </div>
              ...
              >
                <div className="flex justify-between items-center text-slate-900">
                  <div className="flex items-center space-x-4 text-slate-900">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-slate-400 text-slate-400">{o.time}</div>
                    </div>
                  </div>
                  <div className="flex items-center space-x-6 text-slate-900">
                    <div className="text-right text-slate-900 text-slate-900">
                      <div className="text-lg font-black text-slate-800 text-slate-800">${o.total}</div>
                ...
                {expandedOrderId === o.id && (
                  <div className="mt-4 pt-4 border-t border-slate-100 text-[10px] space-y-1 text-slate-900">
                    ...
                    <div className="flex justify-between text-red-400 text-red-400"><span>折扣</span><span>-${o.discount || 0}</span></div>
                    {o.paymentMethod === 'Cash' && (<div className="flex justify-between text-slate-400 text-slate-400"><span>實收/找零</span><span>${o.cashReceived || 0} / ${o.change || 0}</span></div>)}
                    {renderItems(o.items)}
                  </div>
                )}
              </div>
  ...
  return (
    <div className="max-w-4xl text-slate-900 h-full flex flex-col overflow-hidden text-slate-900 text-slate-900">
      ...
      <div className="grid gap-3 overflow-y-auto flex-1 pr-2 pb-10 scrollbar-thin text-slate-900 text-slate-900">
        {menu.map(item => (
          <div key={item.id} className={`bg-white px-8 py-5 rounded-2xl border flex justify-between items-center shadow-sm shrink-0 transition-all text-slate-900 text-slate-900 ${item.isAvailable ? 'border-slate-50 opacity-100' : 'border-slate-200 bg-slate-50/50 opacity-60 grayscale-[0.5]'}`}>
            <div className="flex items-center space-x-4 text-slate-900 text-slate-900">
              <div className={`p-2 rounded-lg ${item.isAvailable ? 'bg-slate-100 text-slate-400 text-slate-400' : 'bg-red-50 text-red-300'}`}>
            ...
            <div className="flex items-center space-x-6 text-slate-900 text-slate-900">
              ...
              <div className="flex space-x-2 border-l pl-6 border-slate-100 items-center text-slate-900 text-slate-900">
                <button
                  ...
                  className={`p-2 rounded-lg transition-colors ${item.isAvailable ? 'text-slate-300 hover:text-blue-500 text-slate-300' : 'text-blue-500 hover:text-blue-600 bg-blue-50 text-blue-500'}`}
                >
                  {item.isAvailable ? <Eye size={18} /> : <EyeOff size={18} />}
                </button>
                <button onClick={() => { setEditingId(item.id); setNewItem({ name: item.name, price: item.price.toString(), category: item.category }) }} className="text-slate-300 hover:text-amber-500 p-2 transition-colors text-slate-300"><Edit2 size={18} /></button>
                <button onClick={() => setMenu(menu.filter(m => m.id !== item.id))} className="text-slate-300 hover:text-red-500 p-2 transition-colors text-slate-300"><Trash2 size={20} /></button>
              </div>
            </div>
          </div>
        ))}
    ...
    <div className="max-w-5xl text-slate-900 h-full flex flex-col overflow-hidden text-slate-900">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10 shrink-0 text-slate-900">
        <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-10 rounded-3xl text-white shadow-xl flex flex-col justify-between min-h-[220px]">
          ...
          <button type="button" onClick={handleDailyClosing} className="mt-6 flex items-center justify-center space-x-2 bg-white/20 hover:bg-white/30 px-6 py-3 rounded-xl font-bold border border-white/30 text-white text-white text-white"><CalendarCheck size={20} /><span>執行日結結帳</span></button>
        </div>
        <div className="bg-white p-10 rounded-3xl border border-slate-100 flex flex-col justify-center shadow-sm text-slate-900 text-slate-900 text-slate-900">
          <p className="text-slate-400 text-sm font-bold uppercase tracking-widest mb-2 text-slate-400 text-slate-400">今日待結筆數</p>
          <h3 className="text-5xl font-black text-slate-800 text-slate-800 text-slate-800">{orders.filter(o => o.paymentStatus === 'pending').length} <span className="text-xl font-normal text-slate-300 text-slate-300 text-slate-300">筆</span></h3>
        </div>
      </div>

      <div className="flex space-x-4 mb-6 border-b border-slate-200 shrink-0 text-slate-900 text-slate-900">
        <button type="button" onClick={() => setShowHistory(false)} className={`pb-4 px-4 font-bold transition-all border-b-2 ${!showHistory ? 'border-blue-600 text-blue-600 text-blue-600' : 'border-transparent text-slate-400 text-slate-400'}`}>日報彙整</button>
        <button type="button" onClick={() => setShowHistory(true)} className={`pb-4 px-4 font-bold transition-all border-b-2 ${showHistory ? 'border-blue-600 text-blue-600 text-blue-600' : 'border-transparent text-slate-400 text-slate-400 text-slate-400'}`}>交易明細</button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pr-2 pb-10 scrollbar-thin text-slate-900 text-slate-900">
        {!showHistory ? (
              ...
              return (
                <div key={summary.id} className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-md transition-all text-slate-900 text-slate-900">
                  <div onClick={() => setExpandSummaryId(isExpand ? null : summary.id)} className={`p-6 flex items-center justify-between cursor-pointer ${isExpand ? 'bg-blue-50/50' : ''}`}>
                    ...
                    <div className="flex items-center space-x-8 text-slate-900 text-slate-900"><div className="text-right text-slate-900 text-slate-900 text-slate-900"><div className="text-xs text-slate-400 uppercase font-bold text-slate-400 text-slate-400">總營收</div><div className="text-2xl font-black text-blue-600 text-blue-600">${summary.total}</div></div>{isExpand ? <ChevronUp className="text-slate-300" /> : <ChevronDown className="text-slate-300" />}</div>
                  </div>
                  {isExpand && (
          ...
          <div className="space-y-3">
            <h3 className="text-slate-400 font-bold text-xs uppercase tracking-widest px-2 mb-2 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">歷史交易明細</h3>
            {[...orders].reverse().map(order => {
              const isExpand = expandOrderId === order.id;
              return (
                <div key={order.id} className={`bg-white rounded-2xl border overflow-hidden transition-all text-slate-900 text-slate-900 ${order.status === 'closed' ? 'border-slate-100 opacity-70 text-slate-900' : 'border-blue-200 shadow-blue-50 text-slate-900'}`}>
                  <div onClick={() => setExpandOrderId(isExpand ? null : order.id)} className="flex items-center px-6 py-5 cursor-pointer hover:bg-slate-50 transition-colors text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                    <div className="flex-1 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><div className="font-bold text-slate-700 flex items-center text-slate-700 text-slate-700 text-slate-700">#{order.orderNo || 'N/A'} - {order.date} <span className={`ml-3 text-[10px] px-2 py-0.5 rounded font-bold ${order.orderType === 'takeOut' ? 'bg-orange-100 text-orange-600' : 'bg-blue-100 text-blue-600'}`}>{order.orderType === 'takeOut' ? '外帶' : '內用'}</span>
                    ...
                    <div className="text-xl font-black text-blue-600 mr-6 text-blue-600 text-blue-600 text-blue-600 text-blue-600">${order.total}</div><ChevronRight className={`text-slate-300 transition-transform ${isExpand ? 'rotate-90' : ''}`} />
                  </div>
                  {isExpand && <div className="px-10 py-6 bg-slate-50 border-t border-slate-100 animate-in fade-in text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><div className="space-y-2 text-slate-900 text-slate-900 text-slate-900 text-slate-900">{renderItemDetails(order.items)}</div><div className="mt-4 pt-4 border-t flex justify-between font-black text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><span>總計金額</span><span className="text-blue-600 text-blue-600 text-blue-600">${order.total}</span></div></div>}
                </div>
              );
            })}
          </div>
...
const SettingsPage = () => {
  ...
  return (
    <div className="max-w-2xl text-slate-900 h-full overflow-hidden text-slate-900 text-slate-900 text-slate-900">
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden text-slate-900 text-slate-900">
        <div className="p-8 space-y-8 text-slate-900 text-slate-900">
          ...
          <hr className="border-slate-100 text-slate-900" />
          <section>
            <div className="flex justify-between items-center mb-4 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
              ...
              <div className="bg-slate-100 p-1 rounded-xl flex text-slate-900 text-slate-900 text-slate-900 text-slate-900"><button type="button" onClick={() => updateConfig('dineInMode', 'prePay')} className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${config.dineInMode === 'prePay' ? 'bg-white text-blue-600 shadow-sm text-blue-600' : 'text-slate-400 text-slate-400'}`}>先結帳</button><button type="button" onClick={() => updateConfig('dineInMode', 'postPay')} className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${config.dineInMode === 'postPay' ? 'bg-white text-blue-600 shadow-sm text-blue-600' : 'text-slate-400 text-slate-400'}`}>後結帳</button></div>
            </div>
            <div className={`p-4 rounded-2xl flex items-start space-x-3 text-slate-900 text-slate-900 text-slate-900 ${config.dineInMode === 'prePay' ? 'bg-blue-50 text-blue-700 text-blue-700 text-blue-700' : 'bg-amber-50 text-amber-700 text-amber-700 text-amber-700'}`}>{config.dineInMode === 'prePay' ? <CheckCircle2 size={20} className="shrink-0 mt-0.5 text-blue-700 text-blue-700 text-blue-700" /> : <Clock size={20} className="shrink-0 mt-0.5 text-amber-700 text-amber-700 text-amber-700" />}<p className="text-xs leading-relaxed font-medium text-slate-900 text-slate-900 text-slate-900">{config.dineInMode === 'prePay' ? "模式：【先結帳】。點餐後需立即收款。" : "模式：【後結帳】。點餐後進入「訂單管理」，離開前收款。"}</p></div>
          </section>
        </div>
        <div className="bg-slate-50 p-6 border-t border-slate-100 text-center text-slate-900 text-slate-900 text-slate-900"><p className="text-xs text-slate-400 flex items-center justify-center text-slate-400 text-slate-400 text-slate-400"><Sliders size={14} className="mr-2 text-slate-400 text-slate-400" /> 設定立即生效</p></div>
      </div>
    </div>
  );
};
