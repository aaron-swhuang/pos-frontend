        ...
        <div className="flex-1 p-8 flex flex-col min-h-0 bg-white">
              ...
              <div key={method.id}>
                {method.enabled ? (
                  <button
                    ...
                    className={`w-full flex items-center justify-center p-4 rounded-2xl border-2 transition-all gap-3 ${paymentMethod === method.id ? 'border-blue-500 bg-blue-50 text-blue-600 shadow-sm' : 'border-slate-100 text-slate-400 hover:bg-slate-50'
                      }`}
                  >
                    {method.icon}
          ...
          <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-0">
            <div className="space-y-4">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest text-slate-400">金額輸入</h3>
              {paymentMethod === 'Cash' ? (
                <div className="space-y-4">
                  <div
    ...
    <div className="flex flex-col lg:flex-row gap-8 text-slate-900 h-full overflow-hidden">
          ...
          {filteredMenu.map(item => (
            <button
              ...
              className={`p-6 rounded-2xl shadow-sm border transition-all text-left group h-fit relative overflow-hidden ${item.isAvailable
                ? 'bg-white border-slate-100 hover:border-blue-500 hover:shadow-md text-slate-800'
                : 'bg-slate-50 border-slate-200 cursor-not-allowed grayscale-[0.8] text-slate-400'
                }`}
            >
              <div className={`font-bold mb-1 truncate ${item.isAvailable ? 'text-slate-800 group-hover:text-blue-600' : ''}`}>
                {item.name}
              </div>
              <div className={`font-black text-xl ${item.isAvailable ? 'text-blue-600' : ''}`}>
                ${item.price}
              </div>
              {!item.isAvailable && (
      ...
      <div className="w-full lg:w-96 flex-shrink-0 bg-white rounded-3xl shadow-xl flex flex-col border border-slate-100 h-full overflow-hidden text-slate-900">
        <div className="p-6 border-b flex flex-col space-y-4 bg-slate-50/50 rounded-t-3xl shrink-0">
          ...
          <div className="grid grid-cols-2 gap-2 bg-white p-1 rounded-xl border border-slate-200 shadow-inner text-slate-900 font-bold font-bold">
        ...
        <div className="p-6 bg-slate-900 text-white rounded-b-3xl shrink-0 text-white">
          ...
          <button
            ...
            className={`w-full py-4 rounded-xl font-bold transition-all active:scale-95 shadow-lg text-white ${orderType === 'dineIn' && config.dineInMode === 'postPay' ? 'bg-amber-500 hover:bg-amber-600 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white'
              }`}
          >
    ...
    <div className="space-y-1 mt-4 pt-4 border-t border-slate-100 text-slate-900">
      {(items || []).map((item, idx) => (
        <div key={idx} className="flex justify-between items-center text-xs text-slate-900">
          <span className="text-slate-500 font-medium text-slate-500">{item.name} x {item.quantity || 1}</span>
          <span className="font-bold text-slate-700 text-slate-700">${(item.price || 0) * (item.quantity || 1)}</span>
        </div>
      ))}
    </div>
  );
    ...
    <div className="max-w-6xl text-slate-900 h-full flex flex-col overflow-hidden text-slate-900">
      <div className="flex justify-between items-end mb-8 shrink-0 text-slate-900 text-slate-900">
        ...
        <div className="flex space-x-4 text-slate-900 font-bold">
          <div className="bg-amber-50 px-6 py-3 rounded-2xl border border-amber-100 text-right text-amber-700"><p className="text-[10px] font-bold text-amber-500 uppercase text-amber-500 uppercase">待結總額</p><p className="text-xl font-black">${pendingOrders.reduce((s, o) => s + o.total, 0)}</p></div>
          <div className="bg-blue-50 px-6 py-3 rounded-2xl border border-blue-100 text-right text-blue-700"><p className="text-[10px] font-bold text-blue-500 uppercase text-blue-500 uppercase">今日已收</p><p className="text-xl font-black">${finishedOrders.reduce((s, o) => s + o.total, 0)}</p></div>
        </div>
      </div>
                ...
                <div className="flex justify-between items-center text-slate-900">
                  <div>
                    <div className="flex items-center space-x-2 mb-1 text-slate-900 text-slate-900">
                      ...
                      <span className="bg-amber-100 text-amber-700 text-[10px] px-2 py-0.5 rounded-full font-bold uppercase text-amber-700">內用後結</span>
                    </div>
                    <div className="text-xs text-slate-400 flex items-center text-slate-400 text-slate-400"><Clock size={12} className="mr-1" /> {o.time}</div>
                  </div>
                  <div className="flex items-center space-x-6">
                    ...
                    >
                      <Wallet size={18} className="mr-2 text-white" /> 結帳
                    </button>
                    {expandedOrderId === o.id ? <ChevronUp className="text-slate-300" /> : <ChevronDown className="text-slate-300 text-slate-300" />}
                  </div>
                </div>
              ...
              >
                <div className="flex justify-between items-center text-slate-900">
                  <div className="flex items-center space-x-4 text-slate-900 text-slate-900">
                    <div className={`p-2 rounded-lg ${o.orderType === 'takeOut' ? 'bg-orange-50 text-orange-500 text-orange-500' : 'bg-blue-50 text-blue-500 text-blue-500'}`}>
      ...
      <form onSubmit={handleSubmit} className={`bg-white p-8 rounded-3xl shadow-sm border transition-all mb-8 shrink-0 ${editingId ? 'border-amber-400 ring-4 ring-amber-50' : 'border-slate-100'}`}>
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 text-slate-900 text-slate-900">
          ...
          <div className="sm:col-span-2 flex items-end gap-2 text-white">
            ...
            {editingId && <button type="button" onClick={() => { setEditingId(null); setNewItem({ name: '', price: '', category: '' }) }} className="bg-slate-100 p-3 rounded-xl text-slate-400 hover:bg-slate-200 transition-colors text-slate-400 text-slate-400"><X size={20} /></button>}
          </div>
        </div>
      </form>
          ...
          <div key={item.id} className={`bg-white px-8 py-5 rounded-2xl border flex justify-between items-center shadow-sm shrink-0 transition-all text-slate-900 text-slate-900 ${item.isAvailable ? 'border-slate-50 opacity-100' : 'border-slate-200 bg-slate-50/50 opacity-60 grayscale-[0.5]'}`}>
            ...
            <div className="flex items-center space-x-6 text-slate-900 text-slate-900">
              ...
              <div className="flex space-x-2 border-l pl-6 border-slate-100 items-center text-slate-900 text-slate-900">
                ...
                <button type="button" onClick={() => { setEditingId(item.id); setNewItem({ name: item.name, price: item.price.toString(), category: item.category }) }} className="text-slate-300 hover:text-amber-500 p-2 transition-colors text-slate-300 text-slate-300"><Edit2 size={18} /></button>
                <button type="button" onClick={() => setMenu(menu.filter(m => m.id !== item.id))} className="text-slate-300 hover:text-red-500 p-2 transition-colors text-slate-300 text-slate-300"><Trash2 size={20} /></button>
              </div>
            </div>
          </div>
        ))}
    ...
    <div className="max-w-5xl text-slate-900 h-full flex flex-col overflow-hidden text-slate-900 text-slate-900">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10 shrink-0 text-slate-900 text-slate-900">
        <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-10 rounded-3xl text-white shadow-xl flex flex-col justify-between min-h-[220px]">
          ...
          <button type="button" onClick={handleDailyClosing} className="mt-6 flex items-center justify-center gap-2 bg-white/20 hover:bg-white/30 px-6 py-3 rounded-xl font-bold border border-white/30 text-white text-white text-white text-white"><CalendarCheck size={20} /><span>執行日結結帳</span></button>
        </div>
        <div className="bg-white p-10 rounded-3xl border border-slate-100 flex flex-col justify-center shadow-sm text-slate-900">
          <p className="text-slate-400 text-sm font-bold uppercase tracking-widest mb-2 text-slate-400 text-slate-400">今日支付分佈</p>
      ...
      <div className="flex space-x-4 mb-6 border-b border-slate-200 shrink-0 text-slate-900 text-slate-900">
        <button type="button" onClick={() => setShowHistory(false)} className={`pb-4 px-4 font-bold transition-all border-b-2 ${!showHistory ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-400 text-slate-400'}`}>日報彙整</button>
        <button type="button" onClick={() => setShowHistory(true)} className={`pb-4 px-4 font-bold transition-all border-b-2 ${showHistory ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-400 text-slate-400'}`}>交易明細</button>
      </div>
              ...
              return (
                <div key={summary.id} className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-md transition-all text-slate-900 text-slate-900">
                  <div onClick={() => setExpandSummaryId(expandSummaryId === summary.id ? null : summary.id)} className={`p-6 flex items-center justify-between cursor-pointer ${expandSummaryId === summary.id ? 'bg-blue-50/50' : ''}`}>
                    ...
                    <div className="flex items-center space-x-8 text-slate-900 text-slate-900"><div className="text-right text-slate-900 text-slate-900"><div className="text-xs text-slate-400 uppercase font-bold text-slate-400 text-slate-400 text-slate-400">總營收</div><div className="text-2xl font-black text-blue-600 text-blue-600">${summary.total}</div></div>{expandSummaryId === summary.id ? <ChevronUp className="text-slate-300" /> : <ChevronDown className="text-slate-300 text-slate-300" />}</div>
                  </div>
...
const SettingsPage = () => {
  ...
  return (
    <div className="max-w-2xl text-slate-900 h-full overflow-hidden text-slate-900 text-slate-900 text-slate-900">
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden text-slate-900 text-slate-900 text-slate-900">
