  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8">
      <div className="flex-1 text-slate-900 flex flex-col min-h-0">
        ...
        {/* 品項網格 */}
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 mb-10 overflow-y-auto max-h-[calc(100vh-420px)] pr-2">
...
const AdminPage = () => {
  ...
  return (
    <div className="max-w-4xl text-slate-900">
      ...
      <form onSubmit={handleAdd} className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 grid grid-cols-1 sm:grid-cols-12 gap-4 mb-8">
        <div className="sm:col-span-5"><label className="text-xs font-bold text-slate-400 uppercase mb-1 block">商品名稱</label><input className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 font-medium" placeholder="例如：焦糖瑪奇朵" value={newItem.name} onChange={e => setNewItem({ ...newItem, name: e.target.value })} /></div>
        <div className="sm:col-span-3"><label className="text-xs font-bold text-slate-400 uppercase mb-1 block">分類</label><input className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 font-medium" placeholder="咖啡 / 甜點" value={newItem.category} onChange={e => setNewItem({ ...newItem, category: e.target.value })} /></div>
        <div className="sm:col-span-2"><label className="text-xs font-bold text-slate-400 uppercase mb-1 block">價格</label><input type="number" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 font-medium" placeholder="0" value={newItem.price} onChange={e => setNewItem({ ...newItem, price: e.target.value })} /></div>
        <button type="submit" className="sm:col-span-2 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 transition-all flex items-center justify-center mt-auto h-12 shadow-lg"><Plus size={20} className="mr-1" /> 新增</button>
      </form>
      <div className="grid gap-3">
        {menu.map(item => (
          <div key={item.id} className="bg-white px-8 py-5 rounded-2xl border border-slate-50 flex justify-between items-center shadow-sm text-slate-900">
            <div className="flex items-center space-x-4 text-slate-900"><div className="bg-slate-100 p-2 rounded-lg text-slate-400"><Tag size={16} /></div><div><div className="font-bold text-slate-700">{item.name}</div><div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{item.category || '未分類'}</div></div></div>
            <div className="flex items-center space-x-10 text-slate-900"><span className="text-blue-600 font-black text-xl">${item.price}</span><button onClick={() => setMenu(menu.filter(m => m.id !== item.id))} className="text-slate-200 hover:text-red-500 transition-colors"><Trash2 size={20} /></button></div>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="max-w-5xl text-slate-900">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10 text-slate-900">
        <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-10 rounded-3xl text-white shadow-xl flex flex-col justify-between min-h-[220px]">
          <div><p className="opacity-70 text-sm font-bold uppercase tracking-widest mb-2 text-white/80">今日營收</p><h3 className="text-5xl font-black text-white">${todayRevenue}</h3></div>
      ...
      <div className="flex space-x-4 mb-6 border-b border-slate-200 text-slate-900">
      ...
      {!showHistory ? (
            ...
            return (
              <div key={summary.id} className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-md transition-all text-slate-900">
                <div onClick={() => setExpandSummaryId(isExpand ? null : summary.id)} className={`p-6 flex items-center justify-between cursor-pointer ${isExpand ? 'bg-blue-50/50' : ''}`}>
                  ...
                  <div className="flex items-center space-x-8 text-slate-900"><div className="text-right"><div className="text-xs text-slate-400 uppercase font-bold">總營收</div><div className="text-2xl font-black text-blue-600">${summary.total}</div></div>{isExpand ? <ChevronUp className="text-slate-300" /> : <ChevronDown className="text-slate-300" />}</div>
                </div>
                {isExpand && (
                  <div className="px-10 py-8 bg-slate-50 border-t border-slate-100 space-y-8 text-slate-900">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      ...
                      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-center"><span className="text-xs text-slate-400 uppercase font-bold block mb-1">平均客單價</span><span className="text-3xl font-black text-slate-800">${summary.orderCount > 0 ? (summary.total / summary.orderCount).toFixed(0) : 0}</span><span className="text-xs text-slate-400 mt-2 italic">共計 {summary.orderCount} 筆交易</span></div>
                    </div>
                    <div className="border-t border-slate-200 pt-8 text-slate-900"><h4 className="text-sm font-bold text-slate-500 mb-4 flex items-center"><Receipt size={16} className="mr-2 text-blue-500" /> 原始訂單明細</h4><div className="space-y-2">{summaryOrders.map((order) => { const isOrderExpand = expandOrderId === order.id; return (<div key={order.id} className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm"><div onClick={(e) => { e.stopPropagation(); setExpandOrderId(isOrderExpand ? null : order.id); }} className="flex items-center px-6 py-4 cursor-pointer hover:bg-slate-50 transition-colors"><div className="flex flex-col flex-1"><span className="text-sm font-bold text-slate-700">號碼 #{order.orderNo || 'N/A'}</span><span className="text-[10px] text-slate-400">{order.time}</span></div><div className="flex-1 text-slate-900">{order.orderType === 'takeOut' ? <span className="bg-orange-100 text-orange-600 text-[10px] px-2 py-1 rounded-md font-bold">外帶</span> : <span className="bg-blue-100 text-blue-600 text-[10px] px-2 py-1 rounded-md font-bold">內用</span>}</div><div className="text-lg font-black text-slate-800 mr-4">${order.total}</div><ChevronRight className={`text-slate-300 transition-transform ${isOrderExpand ? 'rotate-90' : ''}`} size={16} /></div>{isOrderExpand && (<div className="px-10 py-4 bg-slate-50 border-t border-slate-100 animate-in fade-in text-slate-900 text-slate-900"><div className="space-y-1 text-slate-900">{renderItemDetails(order.items)}</div></div>)}</div>); })}</div></div>
                  </div>
                )}
              </div>
            );
        ...
        <div className="space-y-3 text-slate-900">
          <h3 className="text-slate-400 font-bold text-xs uppercase tracking-widest px-2 mb-2 text-slate-900 text-slate-900">歷史交易明細</h3>
...
const MainLayout = () => (
  <div className="flex min-h-screen bg-slate-50 text-slate-900">
    <Sidebar />
    <main className="flex-1 ml-64 p-10"><Routes><Route path="/pos" element={<POSPage />} /><Route path="/admin" element={<AdminPage />} /><Route path="/dashboard" element={<DashboardPage />} /><Route path="*" element={<Navigate to="/pos" />} /></Routes></main>
  </div>
);
