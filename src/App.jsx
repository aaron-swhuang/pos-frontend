...
const CheckoutModal = ({ isOpen, onClose, cartTotal, onConfirm }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-6xl rounded-[2.5rem] shadow-2xl flex flex-col lg:flex-row overflow-hidden max-h-[95vh] text-slate-900 text-slate-900">
        ...
        <div className="flex-1 p-8 flex flex-col min-h-0 bg-white">
          ...
          <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-0 text-slate-900">
            <div className="space-y-4">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest text-slate-400 text-slate-400">金額確認</h3>
              {paymentMethod === 'Cash' ? (
          ...
          <div className="mt-8 flex gap-4 shrink-0">
            ...
            <button
              ...
              className="flex-1 py-5 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-black text-xl shadow-lg shadow-blue-200 transition-all active:scale-95 flex items-center justify-center gap-3 text-white text-white"
            >
              <Wallet size={24} />
              <span>確認完成結帳</span>
            </button>
  ...
  return (
    <div className="w-64 h-screen bg-slate-900 text-white fixed left-0 top-0 flex flex-col border-r border-slate-800 text-white">
      <div className="p-8 flex items-center space-x-3 border-b border-slate-800 text-white text-white">
        ...
        <span className="text-xl font-black tracking-tight text-white uppercase truncate text-white">{config.storeName}</span>
      </div>
      ...
      <div className="flex-1 px-4 space-y-2 mt-6 overflow-y-auto text-white text-white">
        {navItems.map(item => (
          <Link
            key={item.path}
            to={item.path}
            className={`flex items-center space-x-3 px-4 py-3 rounded-xl transition-all ${location.pathname === item.path ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/50 text-white' : 'text-slate-400 hover:bg-slate-800 text-white'
              }`}
          >
            {item.icon}
...
const POSPage = () => {
  ...
  const confirmOrder = (paymentData) => {
    // 關鍵修改：根據類別生成序號 (D = Dine-in, T = Take-out)
        ...
        <div className="flex space-x-2 mb-6 overflow-x-auto pb-2 shrink-0 scrollbar-hide">
          {categories.map(cat => (
            <button
              ...
              className={`px-6 py-2 rounded-full whitespace-nowrap font-bold text-sm transition-all border ${selectedCategory === cat ? 'bg-blue-600 text-white border-blue-600 shadow-md text-white' : 'bg-white text-slate-500 border-slate-200 hover:border-blue-400'
                }`}
            >
              {cat}
            </button>
      ...
      <div className="w-full lg:w-96 flex-shrink-0 bg-white rounded-3xl shadow-xl flex flex-col border border-slate-100 h-full overflow-hidden text-slate-900">
        <div className="p-6 border-b flex flex-col space-y-4 bg-slate-50/50 rounded-t-3xl shrink-0">
          ...
          <div className="grid grid-cols-2 gap-2 bg-white p-1 rounded-xl border border-slate-200 shadow-inner text-slate-900 font-bold">
            <button onClick={() => setOrderType('dineIn')} className={`flex items-center justify-center space-x-2 py-2 rounded-lg text-sm transition-all ${orderType === 'dineIn' ? 'bg-slate-900 text-white shadow-md text-white' : 'text-slate-400 hover:bg-slate-50'}`}><Utensils size={16} /><span>內用</span></button>
            <button onClick={() => setOrderType('takeOut')} className={`flex items-center justify-center space-x-2 py-2 rounded-lg text-sm transition-all ${orderType === 'takeOut' ? 'bg-slate-900 text-white shadow-md text-white' : 'text-slate-400 hover:bg-slate-50'}`}><ShoppingBag size={16} /><span>外帶</span></button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-thin text-slate-900 text-slate-900">
          {cart.map(item => (
            <div key={item.id} className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-slate-900">
              <div className="flex justify-between items-start mb-2 text-slate-900 text-slate-900">
        ...
        <div className="p-6 bg-slate-900 text-white rounded-b-3xl shrink-0">
          <div className="flex justify-between items-center mb-6 text-white text-white">
            ...
            <span className="text-3xl font-black text-white text-white">${cartTotal}</span>
          </div>
          <button
            onClick={checkout}
    ...
    <div className="max-w-6xl text-slate-900 h-full flex flex-col overflow-hidden text-slate-900 text-slate-900">
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-2 gap-8 overflow-hidden">
        <div className="flex flex-col min-h-0 text-slate-900">
          <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center shrink-0 text-slate-400 text-slate-400">
              ...
              >
                <div className="flex justify-between items-center text-slate-900 text-slate-900">
                  ...
                  <div className="flex items-center space-x-6 text-slate-900">
                    <div className="text-right text-slate-900"><p className="text-[10px] font-bold text-slate-400 uppercase text-slate-400">應收</p><p className="text-xl font-black text-slate-900">${o.total}</p></div>
                    <button
                      onClick={(e) => handlePay(e, o)}
                      className="bg-slate-900 text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-600 transition-all flex items-center shadow-md text-white"
                    >
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-2 scrollbar-thin pb-10 text-slate-900">
            {[...finishedOrders].reverse().map(o => (
              <div
                key={o.id}
                onClick={() => toggleExpand(o.id)}
                className={`bg-white p-4 rounded-2xl border transition-all cursor-pointer opacity-80 hover:opacity-100 ${expandedOrderId === o.id ? 'border-blue-400 shadow-md text-blue-400' : 'border-slate-100 shadow-sm'}`}
              >
                <div className="flex justify-between items-center text-slate-900 text-slate-900">
                ...
                {expandedOrderId === o.id && (
                  <div className="mt-4 pt-4 border-t border-slate-100 text-[10px] space-y-1 text-slate-900">
                    <div className="flex justify-between text-slate-400 text-slate-400"><span>原價</span><span>${o.total + (o.discount || 0)}</span></div>
...
const AdminPage = () => {
  ...
  return (
    <div className="max-w-4xl text-slate-900 h-full flex flex-col overflow-hidden text-slate-900 text-slate-900 text-slate-900">
      <h2 className="text-2xl font-bold mb-8 shrink-0 text-slate-900 text-slate-900">菜單設計維護</h2>
      <form onSubmit={handleSubmit} className={`bg-white p-8 rounded-3xl shadow-sm border transition-all mb-8 shrink-0 ${editingId ? 'border-amber-400 ring-4 ring-amber-50' : 'border-slate-100'}`}>
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-4">
          ...
          <div className="sm:col-span-2 flex items-end gap-2 text-white">
            <button type="submit" className={`flex-1 rounded-xl font-bold h-12 transition-all flex items-center justify-center gap-2 shadow-lg text-white ${editingId ? 'bg-amber-500 hover:bg-amber-600' : 'bg-slate-900 hover:bg-slate-800 text-white'}`}>
          ...
          <div key={item.id} className={`bg-white px-8 py-5 rounded-2xl border flex justify-between items-center shadow-sm shrink-0 transition-all text-slate-900 text-slate-900 ${item.isAvailable ? 'border-slate-50 opacity-100' : 'border-slate-200 bg-slate-50/50 opacity-60 grayscale-[0.5]'}`}>
            ...
            <div className="flex items-center space-x-6 text-slate-900 text-slate-900">
              ...
              <div className="flex space-x-2 border-l pl-6 border-slate-100 items-center text-slate-900 text-slate-900">
                ...
                <button onClick={() => setMenu(menu.filter(m => m.id !== item.id))} className="text-slate-300 hover:text-red-500 p-2 transition-colors text-slate-300 text-slate-300"><Trash2 size={20} /></button>
              </div>
            </div>
          </div>
        ))}
    ...
    <div className="max-w-5xl text-slate-900 h-full flex flex-col overflow-hidden text-slate-900 text-slate-900">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10 shrink-0 text-slate-900">
        <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-10 rounded-3xl text-white shadow-xl flex flex-col justify-between min-h-[220px]">
          ...
          <button type="button" onClick={handleDailyClosing} className="mt-6 flex items-center justify-center gap-2 bg-white/20 hover:bg-white/30 px-6 py-3 rounded-xl font-bold border border-white/30 text-white text-white text-white text-white text-white"><CalendarCheck size={20} /><span>執行日結結帳</span></button>
        </div>
        <div className="bg-white p-10 rounded-3xl border border-slate-100 flex flex-col justify-center shadow-sm text-slate-900">
          ...
          <h3 className="text-5xl font-black text-slate-800 text-slate-800">{orders.filter(o => o.paymentStatus === 'pending').length} <span className="text-xl font-normal text-slate-300 text-slate-300">筆</span></h3>
        </div>
      </div>
      ...
      <div className="flex space-x-4 mb-6 border-b border-slate-200 shrink-0 text-slate-900 text-slate-900">
        ...
        <button type="button" onClick={() => setShowHistory(true)} className={`pb-4 px-4 font-bold transition-all border-b-2 ${showHistory ? 'border-blue-600 text-blue-600 text-blue-600' : 'border-transparent text-slate-400 text-slate-400'}`}>交易明細</button>
      </div>
        ...
        {!showHistory ? (
          <div className="space-y-4 text-slate-900">
            {[...dailySummaries].reverse().map((summary) => {
              ...
              return (
                <div key={summary.id} className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-md transition-all text-slate-900 text-slate-900 text-slate-900">
                  <div onClick={() => setExpandSummaryId(expandSummaryId === summary.id ? null : summary.id)} className={`p-6 flex items-center justify-between cursor-pointer ${expandSummaryId === summary.id ? 'bg-blue-50/50' : ''}`}>
                    <div className="flex items-center space-x-4 text-slate-900"><div className="bg-green-100 text-green-600 p-3 rounded-xl text-green-600"><FileText /></div><div><div className="font-bold text-slate-800 text-lg text-slate-800">{summary.date} 彙整報表</div><div className="text-xs text-slate-400 italic text-slate-400 text-slate-400">{summary.closedAt}</div></div></div>
          ...
          <div className="space-y-3">
            ...
            {[...orders].reverse().map(order => {
              ...
              return (
                <div key={order.id} className={`bg-white rounded-2xl border overflow-hidden transition-all text-slate-900 ${order.status === 'closed' ? 'border-slate-100 opacity-70 text-slate-100' : 'border-blue-200 shadow-blue-50 text-slate-900'}`}>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden text-slate-900 text-slate-900">
        <div className="p-8 space-y-8 text-slate-900 text-slate-900">
          ...
          <section>
            ...
            <div className={`p-4 rounded-2xl flex items-start space-x-3 text-slate-900 ${config.dineInMode === 'prePay' ? 'bg-blue-50 text-blue-700 text-blue-700' : 'bg-amber-50 text-amber-700 text-amber-700'}`}>{config.dineInMode === 'prePay' ? <CheckCircle2 size={20} className="shrink-0 mt-0.5 text-blue-700" /> : <Clock size={20} className="shrink-0 mt-0.5 text-amber-700" />}<p className="text-xs leading-relaxed font-medium text-slate-900 text-slate-900">{config.dineInMode === 'prePay' ? "模式：【先結帳】。點餐後需立即收款。" : "模式：【後結帳】。點餐後進入「訂單管理」，離開前收款。"}</p></div>
          </section>
        </div>
        <div className="bg-slate-50 p-6 border-t border-slate-100 text-center text-slate-900 text-slate-900"><p className="text-xs text-slate-400 flex items-center justify-center text-slate-400 text-slate-400"><Sliders size={14} className="mr-2 text-slate-400" /> 設定立即生效</p></div>
      </div>
    </div>
  );
};
